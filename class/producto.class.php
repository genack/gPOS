...
function AltaDesdePostProducto($esMudo=false) {
  ...
  $servicio     = (isset( $_POST["Servicio"]))? CleanID($_POST["Servicio"]=='on'):false;
...
function genListadoCruzado($IdProducto,$IdTallaje = false,$IdLang=false){	
    ...
    $sql = "SELECT  ges_contenedores.Contenedor,ges_productos.VentaMenudeo,ges_productos.UnidadesPorContenedor,ges_locales.NombreComercial,ges_colores.Color,
        ...
        ges_productos.IdTalla = ges_tallas.IdTalla   INNER JOIN ges_contenedores ON 
    ...
    $sql = "SELECT Talla,SizeOrden FROM ges_tallas,ges_productos WHERE ges_tallas.IdTallaje= '$IdTallaje' AND ges_tallas.IdTalla = ges_productos.IdTalla AND ges_productos.IdProdBase = '$IdProdBase'  AND IdIdioma='$IdLang' AND ges_tallas.Eliminado='0'" .
...
function setChangeColorbyIdColor($IdColor,$Color,$IdIdioma=false){
  if (!$IdIdioma)
    $IdIdioma = getSesionDato("IdLenguajeDefecto");
  $IdIdioma = CleanID($IdIdioma);	
  $sql = "UPDATE ges_colores SET Color = '".$Color."'
          WHERE  ges_colores.IdColor   = '".$IdColor."' 
          AND    ges_colores.IdIdioma  = '".$IdIdioma."' 
          AND    Eliminado = 0";
  $res = query($sql);
  if (!$res) {
...
function setChangeTallabyIdTalla($IdTalla,$Talla,$IdIdioma=false){
  if (!$IdIdioma)
    $IdIdioma = getSesionDato("IdLenguajeDefecto");
  $IdIdioma = CleanID($IdIdioma);	
  $sql = "UPDATE ges_tallas SET Talla = '".$Talla."'
          WHERE  ges_tallas.IdTalla   = '".$IdTalla."' 
          AND    ges_tallas.IdIdioma  = '".$IdIdioma."' 
          AND    Eliminado = 0";
  $res = query($sql);
  if (!$res) {
...
function getDatosProductosExtra($id,$extra=false){
	 ...
	 case 'todos':
	   ...
	   array_push($arreglo, $oPd->get("Servicio"));
	   return $arreglo;
	 ...
	 case 'nombretodos':
	   ...
	   $arreglo["Servicio"] = $oPd->get("Servicio");
	   return $arreglo;
	   break;
	 }
}
