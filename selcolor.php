...
StartXul(_("Elije Propiedades del Producto"));
     ...
     case "nuevocolor":
       ...
       $sql    = "SELECT IdColor FROM ges_colores WHERE  IdFamilia='$idfamilia' AND Color='$color' ";
       $row    = queryrow($sql);
       if (!$row)
	 {
     ...
     case "color":
       ...
       echo "\n</script>\n";						
       echo "<script  type='application/x-javascript' src='js/presentacion.js' />";
	 ...
	 echo "<button label='". $txtalla."' oncommand='UsaTallaje(".$row["IdTallaje"].",".$idfamilia.")'/>";	
       ...
       echo "<script>
            function UsaTallaje(id,idfamilia){		
                document.location.href = 'selcolor.php?modo=talla&amp;IdTallaje='+id+'&amp;idfamilia='+idfamilia;
            }
     ...
     case "nuevatalla":
       ...
       $sql       = "SELECT IdTalla FROM ges_tallas WHERE IdFamilia=$idfamilia AND IdTallaje = $tallaje AND Talla='$talla' ";
       $row       = queryrow($sql);
     ...
     case "talla":		
       ...
       echo "<script  type='application/x-javascript' src='js/subpresentacion.js' />";
       echo "<script>\n";
       echo " var fam =new Object();\n";
       foreach ( $familias as $key=>$value ){ echo "fam[$key] = '$value';\n"; }
